const [refreshTaskList, updateNotification] = (() => {
    ...
    function addSingleTodo(todo, parent, template) {
        ...
        if (todo.date) {
            template
                ...
                .next()
                .replaceWith(
                    `<div class="mdui-list-item-text mdui-list-item-one-line">${FormatTime(new Date(todo.date))}</div>`
                );
        } else {
    ...
    return [
        /*refreshTaskList*/ async (dontUpdateNotification) => {
            // 清空任务列表并遍历全部再添加实现刷新
                    ...
                    task_list
                        ...
                        .sortable({
                            ...
                            onUpdate: function (evt) {
                                ...
                                tasks[index].todos.splice(
                                    evt.newIndex - 1,
                                    0,
                                    tasks[index].todos.splice(evt.oldIndex - 1, 1)[0]
                                );
                                saveTasks();
                            },
                        });
            ...
            $("#task-list input").on("click", (e) => {
                ...
                if (item.parent("#task-list").length) {
                    ...
                    const sub_index = item.index(
                        `#task-list > label:nth-of-type(${root_index + 1}) .mdui-list > label`
                    );
...
function openTodoDialog(menuTarget) {
    ...
    if (typeof nowTask.notify === "number") {
    ...
    date.on("input", () => {
        if (date.val() !== "") {
            // 允许勾选 提醒我
            notify_checkbox.removeAttr("disabled");
        }
    });
    // 提醒我 checkbox 切换
...
((dialog) => {
    ...
    ((actions) => {
        ...
        actions.eq(0).on("click", () => {
            SaveFile(
                `JIE-ToDo_tasks-${FormatTime()}.json`,
