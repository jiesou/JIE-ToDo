...
let saveTasks, saveSettings, MergeData;

((_storage) => {
  MergeData = (datatype, a, b, nowarn) => {
      // diffs 供判断是否需要覆盖存储
      let diffs = 0;
      // output 默认为 b
      let output = b || [];
  ...
  tasks = _diffCheckWith.get("tasks");
  saveTasks = () => {
